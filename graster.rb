...
module Graster
  ...
  DEFAULTS = {
    ...
    :on_range =>  [0.0,0.5],        # Luminosity range for which the laser should be on (lower priority than :off_range)
    :off_range => [0.5,1.0],        # Luminosity range for which the laser should be off (higher priority than :on_range)
    :overshoot => 0.5,              # Distance the X axis should travel past the boundary of the image on both sides.
                                    # This needs to be wide enough so that the X axis doesn't start decelerating until
                                    # after it has cleared the image.
    :offset => [1.0,1.0],           # X,Y location for the bottom left corner of the image. The X component in this value
                                    # must be larger than :overshoot.
    :tile_count => [1,1],           # Number of times to repeat the image in the X and Y axes, respectively.
                                    # Unless stated otherwise, a repeated image is treated as one single image in every respect.
    :tile_size => [nil,nil],        # The X and Y spatial intervals at which to repeat tiles. Any nil interval defaults to
                                    # the size of the image on that axis, as derived from the bitmap input.
  ...
  def load_config_file pn
    File.exist?(pn) and self.config = YAML.load_file(pn)
  end

  def draw? pix
    @config[:on_range] === pix && !(@config[:off_range] === pix)
  end

  def pix_to_spans pix
    spans = []
